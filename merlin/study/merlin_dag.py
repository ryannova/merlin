...
class DAG(nx.DiGraph):
    ...
    def remove_edge(self, src, dest):
        if src not in self.nodes:
            logger.warning(f"Attempted to remove an edge ({src}, {dest}), but {src} does not exist.")
            return

        if dest not in self.nodes:
            logger.warning(f"Attempted to remove an edge from ({src}, {dest}), but {dest} does not exist.")
            return

        if (src, dest) not in self.edges:
            logger.warning(f"Attempted to remove edge ({src}, {dest}), which does not exist.")

        super().remove_edge(src, dest)
        logging.debug(f"Removed edge ({src}, {dest}).")

    def get_adjacency_matrix(self):
        return nx.linalg.graphmatrix.adjacency_matrix(self)
